 ...
 */
class ReleaseAutomationDeploymentJarJvmPureAppReleaseStrategy extends AbstractReleaseAutomationDeploymentStrategy implements IDeploymentStrategy, Serializable {


    ReleaseAutomationDeploymentJarJvmPureAppReleaseStrategy(IBinariesRepository repository, Object properties) {
    ...
    protected LinkedHashMap<String, String> createDeploymentPlanProperties(Environment environment, pom) {
        return [
                ...
                "sshRemoteScriptArgs": "${[repository.getArtefactURL("jar", VersionType.RELEASE), repository.getEnvPropertiesURL(environment), environment.getEnvironmentNameValue()].join(" ")}"
        ]
