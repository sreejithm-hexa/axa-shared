 ...
 */
class ReleaseAutomationDeploymentZipJvmPureAppDevelopmentStrategy extends AbstractReleaseAutomationDeploymentStrategy implements IDeploymentStrategy, Serializable {


    ReleaseAutomationDeploymentZipJvmPureAppDevelopmentStrategy(IBinariesRepository repository, Object properties) {
    ...
    protected LinkedHashMap<String, String> createDeploymentPlanProperties(Environment environment, pom) {
        return [
                ...
                "sshRemoteScriptArgs": "${[repository.getArtefactURL("zip", VersionType.DEVELOPMENT), repository.getEnvPropertiesURL(environment, VersionType.DEVELOPMENT), environment.getEnvironmentNameValue()].join(" ")}"
        ]
