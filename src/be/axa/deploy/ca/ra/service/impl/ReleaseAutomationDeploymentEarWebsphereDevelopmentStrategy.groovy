 ...
 */
class ReleaseAutomationDeploymentEarWebsphereDevelopmentStrategy extends AbstractReleaseAutomationDeploymentStrategy implements IDeploymentStrategy, Serializable {


    ReleaseAutomationDeploymentEarWebsphereDevelopmentStrategy(IBinariesRepository repository, Object properties) {
    ...
    protected LinkedHashMap<String, String> createDeploymentPlanProperties(Environment environment, pom) {
        return [
                ...
                "sshRemoteScriptArgs": "${[repository.getArtefactURL("ear", VersionType.DEVELOPMENT), repository.getEnvPropertiesURL(environment, VersionType.DEVELOPMENT), environment.getEnvironmentNameValue()].join(" ")}"
        ]
