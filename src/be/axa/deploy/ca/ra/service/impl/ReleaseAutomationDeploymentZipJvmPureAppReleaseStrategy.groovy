 ...
 */
class ReleaseAutomationDeploymentZipJvmPureAppReleaseStrategy extends AbstractReleaseAutomationDeploymentStrategy implements IDeploymentStrategy, Serializable {


    ReleaseAutomationDeploymentZipJvmPureAppReleaseStrategy(IBinariesRepository repository, Object properties) {
    ...
    protected LinkedHashMap<String, String> createDeploymentPlanProperties(Environment environment, pom) {
        return [
                ...
                "sshRemoteScriptArgs": "${[repository.getArtefactURL("zip", VersionType.RELEASE), repository.getEnvPropertiesURL(environment), environment.getEnvironmentNameValue()].join(" ")}"
        ]
